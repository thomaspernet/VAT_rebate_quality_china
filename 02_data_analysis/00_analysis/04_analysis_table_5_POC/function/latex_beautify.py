...
resolution = 150):
    ...
    with open(table_out, 'r') as file:
        ...
        lines = lines.replace('lag\\_foreign\\_export\\_share\\_ckjr',
                              '\\text{lag foreign export share}_{ckjr}^R')

        lines = lines.replace('lag\\_foreign\\_export\\_share\\_ckr',
                              '\\text{lag foreign export share}_{ckr}^R')

        lines = lines.replace('lag\\_soe\\_export\\_share\\_ckjr',
                              '\\text{lag SOE export share}_{ckjr}^R')
